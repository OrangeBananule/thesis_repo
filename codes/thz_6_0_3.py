...
while True:
    ...
    if counter%(10) == 0 and counter <=  800:
        ...
        ax1.plot([np.pi/2,np.pi*3/2],np.array([1,1])*15*6e3,c='black') #surface
        ...
        ax2.plot([np.pi/2,np.pi*3/2],np.array([1,1])*15*12e3,c='black') #surface
    ...
    if counter%(10) == 0 and counter > 800:
        ...
        ax1.grid(True)
        ax1.set_title(r"$|E_{TE}|^2$ transmitted", va='bottom')
        ax1.set_ylim(0,35.5*12e3)
        ylocs = np.linspace(0,35,6)*12e3
        ax1.set_yticks(ylocs)
        ax1.set_yticklabels(ylocs/12e3)
        ax1.text(np.pi*(1.125),(35 + 7.04)*12e3,'*scale by 12e3')
        ax1.plot([np.pi/2,np.pi*3/2],np.array([1,1])*35*12e3,c='black') #surface
        
        ax2.set_theta_zero_location("N")
        ax2.grid(True)
        ax2.set_title(r"$|E_{TM}|^2$ transmitted", va='bottom')
        ax2.set_ylim(0,35.5*24e3)
        ylocs = np.linspace(0,35,6)*24e3
        ax2.set_yticks(ylocs)
        ax2.set_yticklabels(ylocs/24e3)
        ax2.text(np.pi*(1.125),(35 + 7.04)*24e3,'*scale by 24e3')
        ax2.plot([np.pi/2,np.pi*3/2],np.array([1,1])*35*24e3,c='black') #surface
